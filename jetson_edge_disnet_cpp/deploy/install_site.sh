...
mkdir -p "${TARGET_ROOT}"
mkdir -p "${TARGET_ROOT}/jetson_edge_disnet_cpp"
cp -a "${APP_SOURCE_ROOT}/." "${TARGET_ROOT}/jetson_edge_disnet_cpp/"
mkdir -p "${TARGET_ROOT}/config"
cp "${CONFIG_SOURCE}" "${TARGET_ROOT}/config/device.ini"
chmod +x "${TARGET_ROOT}/jetson_edge_disnet_cpp/scripts/run_rtsp_probe.sh"
cp "${TARGET_ROOT}/jetson_edge_disnet_cpp/deploy/rtsp_probe@.service" "${SERVICE_FILE}"
